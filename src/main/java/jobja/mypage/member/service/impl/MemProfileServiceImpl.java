...
public class MemProfileServiceImpl implements MemProfileService {
	...
	public int mypagePost(MemberVO memberVO) {
		...
		result += uploadFile(uploadFile, memberVO.getMemId());
		
		return result;
	}
	
   private int uploadFile(MultipartFile[] uploadFile, String memId) {
		...
		if(uploadFile[0].getOriginalFilename().length()>0) {
			...
			result += this.memProfileMapper.delAtch(memId);
			
			result += this.memProfileMapper.insertAtch(memId);
			...
			for(MultipartFile multipartFile : uploadFile) {
				...
				try {
					...
					atchFileDetailVO.setAfId(memId);	//P.K
...
//					result += this.atchFileDetailMapper.insertAtchFileDetail(atchFileDetailVO);
					result += this.memProfileMapper.updateMyProfile(atchFileDetailVO);
