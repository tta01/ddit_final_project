...
<div class="body-sub-con">
		<h1 class="admin-h1">현직자 인터뷰</h1>
		
		<div class="card-tools" style="display: flex; justify-content: space-between;">
                  <span></span>
<!-- 			<div class="serchbox"> -->
<!--                 <input class="search-input" placeholder="회사명을 검색해주세요." type="text" name="interEntNm" id="entNmInput" value="" style="margin-left: auto;"> -->
...
<!--             </div> -->
		...
		<div class="flex mb-7">
			<p class="listCount">
				<span></span> 총 <strong>${totalInterview}</strong> 건
				<div class="insert ml-auto">
				    <a href="/boardInterView/create">작성하기</a><i class="fa-regular fa-file-lines"></i>
				</div>
		</div>
			<div class="one">
		<table class="table1" cellspacing="0" summary="일반 채용정보에 등록한 회사의 근무지, 업무내용, 회사명, 성별, 근무시간, 급여, 등록일 정보입니다.">
			...
			<tbody>
			<c:forEach var="interviewVO" items="${interviewVOList}" varStatus="stat">
			<input type="hidden" value="${interviewVO.boardId}">
			<input type="hidden" value="${interviewVO.boardTitle}">
				<tr class="firstLine ">
					<td class="title">
					  ${interviewVO.boardTitle}    
					</td>
					<c:choose>
						<c:when test="${interviewVO.boardCntnt != null}">
							<td><a href="/boardInterView/detail?boardId=${interviewVO.boardId}">내용보기</a></td>
						</c:when>
					</c:choose>
					<td class="regDate last"><fmt:formatDate pattern="yyyy년  MM월 dd일"
												value="${interviewVO.boardWritingDt}" /></td>
					<td class="regDate last"><fmt:formatDate pattern="yyyy년  MM월 dd일"
												value="${interviewVO.boardUpdtDt}" /></td>
					<td>
					  ${interviewVO.interEntNm}    
					</td>
					<td>
					  ${interviewVO.interNm}    
					</td>
...
<script type="text/javascript">
            	 ...
            	 Swal.fire({
                     ...
                     $.ajax({
                         ...
                         success: function(result) {
                        	 console.log(result);
                        	 window.location.href = "/admin/adminBoardInterview";
                         }
           			});
            }); 
    ...
    $('#btnSearch').on('click', function(){
    	...
    	window.location.href = '/admin/adminBoardInterview?keyword='+keyword;
    });
</script>