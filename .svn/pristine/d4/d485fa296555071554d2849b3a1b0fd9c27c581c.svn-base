...
<div class="mypage-body-con">
	...
	<div class="body-sub-con">
		<h3 class="admin-h1">신고 목록</h3>
			<div class="biz-tab">
				<a class="biz-tab__item
					<c:if test="${param.sort =='' || param.sort == null }"> biz-tab__item--active</c:if>
				"
					href="/admin/adminReport">전체 
					<span class="biz-tab__number"> ${totalRePort}</span></a> 
					
				<a class="biz-tab__item <c:if test="${param.sort =='success'}"> biz-tab__item--active</c:if>" href="/admin/adminReport?sort=success">승인
					<span class="biz-tab__number"> ${reports}</span>
				</a> 
				<a class="biz-tab__item 
				<c:if test="${param.sort =='waiting'}"> biz-tab__item--active</c:if>
				" href="/admin/adminReport?sort=waiting">승인
					대기<span class="biz-tab__number"> ${reportWating}</span>
				</a> <a class="biz-tab__item border-right
				<c:if test="${param.sort =='false'}"> biz-tab__item--active</c:if>" 
				href="/admin/adminReport?sort=false">승인
				...
				<div class="serchbox ml-auto">
					...
					<button id="btnSearch" type="submit" class="btn btn-sm btn-secondary serch">검색</button>
				</div>
			</div>
						...
						<td>
							<c:choose>
								<c:when test="${reportVO.repResultCntnt != null}">
									<a href="#" class="black btn-sm bg-sig txt-white detailrepcntnt" data-bs-toggle="modal" data-bs-target=".detailmodal">상세보기</a>	
									<input type="hidden" name="repResultCntnt" value="${reportVO.repResultCntnt}">
								</c:when>
								<c:when test="${reportVO.repResultCntnt == null}">
									<a href="#" class="black btn-sm bg-sig txt-white inpBtn" data-bs-toggle="modal" data-bs-target=".notemodal">입력하기</a>		
