...
<div class="mypage-div">
	<div class="mypage-left">
		...
		<ul>
			<li><a href="/entApplication/offerEntList">기업회원 신청내역</a></li>
			<li class="active"><a href="/entApplication/offerReportList">신고내역</a></li>
		</ul>
	</div>
					...
					<table class="table1">
						...
						<tbody>
							<c:forEach var="list" items="${list}" varStatus="stat">
								<tr>
									<td>${list.repSubId}</td>
									<td>${list.repReason}</td>
									<td><fmt:formatDate value="${list.repDt}" pattern="yyyy-MM-dd"/></td>
								<c:if test="${list.repResultCd=='AYN01001'}">
									<td>승인완료</td>
								</c:if>
								<c:if test="${list.repResultCd=='AYN01002'}">
									<td>승인대기</td>
								</c:if>
								<c:if test="${list.repResultCd=='AYN01003'}">
									<td>승인거절</td>
								</c:if>
								</tr>
							</c:forEach>
