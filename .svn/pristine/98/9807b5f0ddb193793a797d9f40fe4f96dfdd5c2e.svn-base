...
<div class="body-sub-con">
	<h1 class="admin-h1">취업 상담</h1>
			
		<div class="card-tools" style="display: flex; justify-content: space-between;">
                  <span></span>
<!-- 	<div class="serchbox"> -->
<!--               <input class="search-input" placeholder="ID를 검색해주세요." type="text" name="interEntNm" id="entNmInput" value="" style="margin-left: auto;"> -->
<!-- 		<a id="btnSearch" href="#" class="btn btn-sm btn-secondary search">검색</a> -->
<!--           </div> -->
<!-- 		</div> -->
	<div id="NormalInfo" class="goodsList goodsJob">
		<p class="listCount mb-7">
			총  <strong>${totalBoardConsult}</strong>건  
		</p>
			<div class="one">
		<table class="table1" summary="일반 채용정보에 등록한 회사의 근무지, 업무내용, 회사명, 성별, 근무시간, 급여, 등록일 정보입니다.">
			<thead>
				<tr>
						<th style="width: 8%;">게시물ID</th>
						<th style="width: 8%;">작성자ID</th>
						<th style="width: 25%;">게시물 제목</th>
						<th style="width: 10%;">게시물 내용</th>
						<th style="width: 12%;">작성일시</th>
						<th style="width: 12%;">수정일시</th>
						<th style="width: 12%;">상담사직군</th>
						<th style="width: 8%;">조회수</th>
						<th style="width: 8%;">비고</th>
				</tr>
			</thead>
			<tbody>
			<c:forEach var="boardEmplConsultVO" items="${boardEmplConsultVOList}" varStatus="stat">
				<tr class="firstLine ">
					<td class="local first " scope="row">
						<div class="L_MyAd_maximumfitness"></div>
						${boardEmplConsultVO.boardId}
					</td>
					<td>
					  ${boardEmplConsultVO.memId}    
					</td>
					<td>
					  ${boardEmplConsultVO.boardTitle}    
					</td>
					<td>
					  <a href="/boardConsulting/detail?boardId=${boardEmplConsultVO.boardId}">상세보기</a>    
					</td>
					<td class="regDate last"><fmt:formatDate pattern="yyyy년  MM월 dd일"
												value="${boardEmplConsultVO.boardWritingDt}" /></td>
					<td class="regDate last"><fmt:formatDate pattern="yyyy년  MM월 dd일"
												value="${boardEmplConsultVO.boardUpdtDt}" /></td>
					<td>
					  ${boardEmplConsultVO.jobCategoryCd}   
					</td>
					<td>
					  ${boardEmplConsultVO.boardCnt}   
					</td>
					<td>
...
<script type="text/javascript">
            	 ...
            	 Swal.fire({
                     ...
                     $.ajax({
                         ...
                         success: function(result) {
                        	 console.log(result);
                        	 window.location.href = "/admin/adminBoardConsult";
                         }
           			});
            }); 
    ...
    $('#btnSearch').on('click', function(){
    	...
    	window.location.href = '/admin/adminBoardConsult?keyword='+keyword;
    });
</script>