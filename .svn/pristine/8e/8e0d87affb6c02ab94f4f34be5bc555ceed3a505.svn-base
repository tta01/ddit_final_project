...
<form:form modelAttribute="memberVO" action="/admin/adminEnterProposal" method="post">
	...
	<div class="biz-tab">
			...
			" href="/admin/adminEnterpriseProposal">전체 <span class="biz-tab__number"> ${totalOfferEnter}</span></a>
                  <a class="biz-tab__item 
				<c:if test="${param.sort == 'success'}"> biz-tab__item--active</c:if>
			 " href="/admin/adminEnterpriseProposal?sort=success">승인 <span class="biz-tab__number"> ${totalEnter}</span></a>
                  <a class="biz-tab__item  
				<c:if test="${param.sort == 'waiting'}"> biz-tab__item--active</c:if>
			" href="/admin/adminEnterpriseProposal?sort=waiting">승인 대기<span class="biz-tab__number"> ${enterWaitCount}</span></a>
                  <a class="biz-tab__item  border-right
				<c:if test="${param.sort == 'false'}"> biz-tab__item--active</c:if>
			" href="/admin/adminEnterpriseProposal?sort=false">승인 취소<span class="biz-tab__number"> ${enterFalseCount}</span></a>
     			...
     			<colgroup>
			       ...
			       <thead>
			           <tr class="uu_uu">
			               ...
			               <th scope="col" class="headdd">기업 직군</th>
			       ...
			       <c:forEach var="memberVO" items="${memberVOList}" varStatus="stat">
				           <tr class="u_u">		
				           		...
				           		<c:choose>
				           			...
				           			<c:when test="${memberVO.entSector == 'MWJ01008'}">
				           				HR 직군
				           			</c:when>
					            ...
					            <td> <!-- 셀 병합을 통해 전체 너비를 차지하도록 설정합니다 -->
			...
			<div class="modal-dialog modal-dialog-scrollable">
				<div class="modal-content">
					...
					<table class="modaltable">
						...
						<tr>
							<th class="thth">기업직군</th>
							<td class="tdtd" id="memWishJobCate"></td>
						</tr>
						<!-- 나머지 항목도 같은 형식으로 추가 -->
						<tr>
							<th class="thth">기업관심키워드</th>
...
// 									        memYn.insertAdjacentHTML('beforeend', '<input type="button" class="Yncondition detailGo list-counting__applicants-btn" value="상태 변경"/>');
	            ...
	            function closeModal() {
	                ...
	                // 페이지 이동
	                window.location.href = "/admin/adminEnterpriseProposal";
	            }
