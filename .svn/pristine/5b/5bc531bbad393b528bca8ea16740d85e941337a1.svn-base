...
<div class="mypage-body-con">
	...
	<div class="body-sub-con">
		<h1 class="admin-h1">기업회원</h1>
		<form:form modelAttribute="memberVO"
			action="/admin/adminEnterpriseMyPage" method="post">

			<div class="card-tools mb-7"
				style="display: flex; align-items: flex-end;">
				
				<h2>기업회원
				</h2>
				<p class="listCount ml-12">
					<strong>(${totalEnter})</strong>
				</p>
		...
		<div class="card-body table-responsive p-0">
			<table class="table table1 table-valign-middle table1">
				<thead>
					<tr>
						<th></th>
						<th>이름</th>
				...
				<tbody>
					...
					<c:forEach var="memberVO" items="${enterVOList}" varStatus="stat">
						<tr>
							<c:choose>
								<c:when
									test="${memberVO.memAfId != null && memberVO.memAfId  != ''}">
									<td><img alt=""
										src="/resources/upload/${memberVO.memAfId}"></td>
								</c:when>
								<c:otherwise>
							...
							<td><input type="hidden" class="memberId"
								value="${memberVO.memId}" /> <input type="hidden"
								class="ComCodeInfoVO" value="${memberVO.memWishJobCate}">
								<input type="hidden" class="ComDetInfoVO"
								value="${memberVO.memWishJob}"> <input type="hidden"
								class="ComDetInfoVO2" value="${memberVO.memIntWord}">

								<button type="button" class="detailGo btn btn-success"
									data-bs-toggle="modal" data-bs-target="#modalDialogScrollable">상세보기</button>
								<button type="button" class="deleteGO btn btn-danger"
									data-bs-toggle="modal" data-bs-target="#modalDelete">삭제</button></td>
		...
		<div class="modal-dialog modal-dialog-scrollable">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<span>회원 정보를 삭제하시면 다시 복구할수 없습니다.</span><br> <span>그래도
						삭제하시겠습니까?</span>
				</div>
				<div class="modal-footer">
					<button id="confirmDelete" type="button" class="btn btn-danger dd"
						data-bs-dismiss="modal">삭제</button>
					<button id="closeModal" type="button" class="btn btn-secondary aa"
						data-bs-dismiss="modal">닫기</button>
				</div>
			</div>
		</div>
	                	...
	                	deleteButton.addEventListener('click', function(event) {
	                    	...
	                    	$.ajax({
	        	                ...
	        	                success: function(result) {
	        	                	...
	        	                	console.log(result);
	        	                	 var redirectURL = "/admin/adminEnterprise";
