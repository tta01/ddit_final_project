...
<!--            </div> -->
	...
	<div class="listCount flex mb-7">
		<span>총 <strong>${totalNews}</strong> 건</span> 
		<div class="insert ml-auto">
		    <a href="/news/create">작성하기</a><i class="fa-regular fa-file-lines"></i>
		</div>
	</div>
		<div class="one">
	<table class="table1" summary="일반 채용정보에 등록한 회사의 근무지, 업무내용, 회사명, 성별, 근무시간, 급여, 등록일 정보입니다.">
		...
		<tbody>
		<c:forEach var="boardNewsVO" items="${newsVOList}" varStatus="stat">
			<tr class="firstLine ">
				...
				<td>
				  <a href="/news/detail?boardId=${boardNewsVO.boardId}">상세보기</a>    
				</td>
				<td class="regDate last"><fmt:formatDate pattern="yyyy년  MM월 dd일"
											value="${boardNewsVO.boardWritingDt}" /></td>
				<td class="regDate last"><fmt:formatDate pattern="yyyy년  MM월 dd일"
											value="${boardNewsVO.boardUpdtDt}" /></td>
				<td>
				  ${boardNewsVO.boardCnt}   
				</td>
				<td>
					<a href="/news/detailUpdate?boardId=${boardNewsVO.boardId}">
						<button type="button" class="detailGo btn btn-sm btn-secondary btn-free" id="btn-free">수정</button>
					</a>
...
<script type="text/javascript">
            	 ...
            	 Swal.fire({
                     ...
                     $.ajax({
                         ...
                         success: function(result) {
                        	 console.log(result);
                        	 window.location.href = "/admin/adminBoardNews";
                         }
           			});
            }); 
    ...
    $('#btnSearch').on('click', function(){
    	...
    	window.location.href = '/admin/adminBoardNews?keyword='+keyword;
    });
</script>