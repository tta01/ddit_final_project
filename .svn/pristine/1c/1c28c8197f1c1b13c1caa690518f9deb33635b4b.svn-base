...
input[type="datetime-local"] {
  ...
  border: 1px solid #ccc;
  font-size: 1rem;
  width: calc(80% - 18px); /* 전체 너비에서 마진 값 제외한 크기로 조정 */
}
...
<script>
    ...
    $(document).ready(() => {
    	...
    	$("#modal-custom").iziModal({
            ...
            let request = $.ajax({
                url: "${pageContext.request.contextPath}/entCalendar/getEvents",
                type: "GET",
                dataType: "json"
            });
            ...
            function event_update(info) {
                ...
                //일정 수정시 서버로 요청
        	    $.ajax({
        	    	url: "${pageContext.request.contextPath}/entCalendar/updateEvent",
        	    	type: "post",
        	    	dataType: "json",
        	    	data: jsondata,
            ...
            request.done((data) => {
               	...
               	const eventColor = {
            	   '기업일정': { backgroundColor: '#60f590', color: '#60f590' },
            	   '면접일정': { backgroundColor: '#B9E2FA', color: '#B9E2FA' },
            	   '채용일정': { backgroundColor: '#FFD2D7', color: '#FFD2D7' }
				};
                    ...
                    select: function (arg) {
                    	...
                    	$('#addEventButton1').on('click',function(){
                    	    ...
                    	    if(eventTitle && startDateTime && endDateTime !=null){
                        	    ...
                        	    //일정 추가 -> 서버로 요청
                        	    $.ajax({
                        	    	url: "${pageContext.request.contextPath}/entCalendar/addEvent",
                        	    	type: "post",
                        	    	dataType: "json",
                    ...
                    eventClick: (event) => {
                        if (confirm(event.event.title + " 일정을 삭제 하시겠습니까?")) {
                            ...
                            $.ajax({
                                ...
                                url: "${pageContext.request.contextPath}/entCalendar/deleteEvent",
                                type: "post",
                                data: data,
                                dataType: "json",
                                contentType: "application/json",
